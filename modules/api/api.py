...
class Api:
    ...
    def list_model_files(self, wallet_address: str):
        BASE_PATH = "models/Lora/user"  # Ensure this path is correct
        user_dir = os.path.join(BASE_PATH, wallet_address, "models")

        if os.path.exists(user_dir) and os.path.isdir(user_dir):
            files = os.listdir(user_dir)
            return {"files": files}
        else:
            raise HTTPException(status_code=404, detail=f"User directory not found at {user_dir}")
    ...
    def notify_backend(self):
        ...
        try:
            ...
            response = requests.post(f"{NEXTJS_BACKEND_URL}/api/runpod/markReady", json={"subscription_id": subscription_id, "status": "ready"})
